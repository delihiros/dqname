...
def parse_args():
  ...
  # check args
  try:
    if args.mode not in ['train', 'test', 'server']: raise ValueError('you must set mode = \'train\' or \'test\'')
...
def serve(args):
  server = msgpackrpc.Server(Serve(args), pack_encoding='utf-8', unpack_encoding='utf-8')
  server.listen(msgpackrpc.Address('localhost', 5000))
  server.start()
...
def main():
  ...
  elif args.mode == 'server': serve(args)
